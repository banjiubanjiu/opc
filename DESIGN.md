...
本项目在 Cohere 基础上做了"暖化"定制,以贴合社区品牌(守望相助、科技向善):

- 画布 `canvas` 从纯白 #ffffff → 暖白 #fdfcfa;hairline 暖化 #e3e0da
- 珊瑚 #ff7759 → #e8663f(加深保对比),仍是小点缀:章节编号、眉题、大引言、EventCard 阶段标签、关键强调
- 新增 `surface-warm` #fbf3ee 极暖淡色带(用于"泼冷水"等需人情味的 section)
- Hero 用细线星座插画(`HeroMark.astro`):一个人(中心)连接产品/用户/客户/伙伴,呼应"一个人也是一家公司"
- 声音区块:首条做大引言,其余双栏规则行,避免单调
- 主按钮仍为近黑 pill;收尾 CTA 带仍为深绿黑 #003c33

优先级提醒(未完成):
1. 报名表单无后端(formEndpoint 为空),数据未收集
2. 无 OG/社交分享标签,微信转发无卡片预览
